...
test('ji open - handles missing configuration', async () => {
  ...
  try {
    ...
    expect(logs.some((log) => log.includes('Configuration error:'))).toBe(true);
    expect(logs.some((log) => log.includes("Run 'ji auth' to set up authentication"))).toBe(true);
