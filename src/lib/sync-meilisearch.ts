...
export async function syncToMeilisearch(options: { clean?: boolean } = {}): Promise<{ indexedItems: number }> {
  ...
  try {
    ...
    const lastSyncResult = db.prepare('SELECT value FROM config WHERE key = ?').get('last_meilisearch_sync') as
      | { value: string }
      | undefined;
    ...
    while (offset < totalCount.count) {
      const queryParams = options.clean || lastSyncTime === 0 ? [batchSize, offset] : [lastSyncTime, batchSize, offset];
