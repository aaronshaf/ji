...
export async function syncToMeilisearch() {
  ...
  try {
    ...
    // Get counts
    const totalCount = contentManager.db.prepare('SELECT COUNT(*) as count FROM searchable_content').get() as {
      count: number;
    };
    ...
    await Promise.all([meilisearch.clearIndex('jira'), meilisearch.clearIndex('confluence')]);
