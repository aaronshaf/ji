    ...
    pipe(
      ...
      Effect.catchAll(error => {
        if (!(error instanceof expectedErrorType)) {
          const errorName = error && typeof error === 'object' && error.constructor 
...
export class TestMocks {
      ...
      (message: string, metadataOrError?: any, metadata?: any) => 
        Effect.sync(() => {
          logs.push({
            level,
            message,
            metadata: metadata || (metadataOrError && !(metadataOrError instanceof Error) ? metadataOrError : undefined),
  ...
  static createMockConfig(config: Partial<AppConfig> = {}): ConfigurationService {
    const mockConfig: AppConfig = {
      ...
      jira: { baseUrl: 'https://test.atlassian.net', email: 'test@example.com', maxResults: 50, timeout: 30000, retries: 3 },
      ...
      logging: { level: 'debug', enableConsole: false, enableFile: false, enableStructured: false, maxFileSize: 1024, maxFiles: 1, enableColors: false, includeStackTrace: false },
      ...
      features: { backgroundSync: false, aiAssistant: false, searchAnalytics: false, cacheWarming: false, performanceMetrics: false },
      ...config
    };
    ...
    return {
      ...
      generate: () => pipe(
        ...
        Effect.flatMap(length => 
          Effect.all(Array(length).fill(0).map(() => elementGenerator.generate()))
        )
      ),
      shrink: (value) => Effect.sync(() => {
    ...
    return pipe(
      ...
      Effect.flatMap(beforeMB => 
        pipe(
          operation,
          Effect.map(result => {
            const afterMB = typeof process !== 'undefined' && process.memoryUsage
...
export class TestEnvironment {
  ...
  runTest<T>(
    ...
    return pipe(
      ...
      Effect.ensuring(
        this.config.cleanup 
          ? pipe(this.fixtures.teardownAll(), Effect.catchAll(() => Effect.succeed(undefined)))
          : Effect.succeed(undefined)
      )
    );
  }
      ...
      Effect.flatMap(shrunkValues => {
        ...
        const findSmallestFailure = (values: T[], index: number): Effect.Effect<T, Error> => {
          ...
          return pipe(
            ...
            Effect.flatMap(passed => 
              passed 
                ? findSmallestFailure(values, index + 1)
...
export const TestUtils = {
   ...
   */
  createFailingEffect: <E>(error: E): Effect.Effect<never, E> =>
    Effect.fail(error),
  ...
  createFlakeyEffect: <T, E>(
    ...
    pipe(
      ...
      Effect.flatMap(shouldFail => 
        shouldFail ? Effect.fail(error) : Effect.succeed(value)
      )
    ),
