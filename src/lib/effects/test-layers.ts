      ...
      execute: <T>(sql: string, params?: unknown[]) =>
        Effect.try({
          try: () => {
            ...
            return (params ? stmt.run(...params) : stmt.run()) as T;
          },
            ...
            pipe(
              Effect.try({
                try: () => db.run('ROLLBACK'),
                  ...
                  new DatabaseError(`Failed to rollback test transaction: ${rollbackError}`)
              }),
...
export class TestHttpClientService {
  ...
  makeService(): HttpClientService {
    ...
    const makeRequest = <T>(url: string, options?: RequestInit) => {
      ...
      const response = typeof expectation.response === 'function'
        ? expectation.response(url, options)
...
// ============= Combined Test Layer =============
export const createTestAppLayer = (
