...
import { DatabaseError, ConfigError, NetworkError } from './errors.js';
...
export const ConfigServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const saveConfig = (config: JiConfig) => Effect.try({
      try: () => {
      ...
      getConfig: pipe(
        ...
        Effect.flatMap((config) =>
          config
            ...
            : Effect.fail(new ConfigError('No configuration found'))
        )
      ),
