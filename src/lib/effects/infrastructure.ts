...
import { 
  ...
  createStreamingSearchService 
} from './search-enhancement.js';
...
export class InfrastructureManager {
  ...
  shutdown(): Effect.Effect<void, never> {
    ...
    return pipe(
      this.infrastructure.logger.info('Shutting down infrastructure'),
      Effect.flatMap(() => Effect.all([
        // Flush logs
        this.infrastructure!.logger.flush(),
        
        // Clear cache if needed
        this.infrastructure!.cache ? 
          pipe(
            this.infrastructure!.cache.getStats(),
            Effect.flatMap(stats => 
              this.infrastructure!.logger.info('Cache stats at shutdown', {
                hits: stats.hits,
                misses: stats.misses,
  ...
  healthCheck(): Effect.Effect<{
    ...
    return pipe(
      ...
      Effect.map(serviceResults => {
        ...
        const status: 'healthy' | 'degraded' | 'unhealthy' = unhealthyCount === 0 
          ? 'healthy' 
          : unhealthyCount < Object.keys(services).length 
            ? 'degraded' 
            : 'unhealthy';
  ...
  private checkCacheHealth(): Effect.Effect<{ status: 'up' | 'down'; error?: string }, never> {
    ...
    return pipe(
      ...
      Effect.catchAll(error => Effect.succeed({ 
        status: 'down' as const, 
        error: typeof error === 'object' && error !== null && 'message' in error ? (error as Error).message : String(error)
      }))
    );
  }
...
export const InfrastructureUtils = {
   ...
   */
  runWithInfrastructure: <T, E>(
    effect: Effect.Effect<T, E, AppInfrastructure>
  ): Effect.Effect<T, E | ConfigError> =>
    pipe(
      getGlobalInfrastructure(),
      Effect.flatMap(infrastructure =>
        pipe(
          effect,
          Effect.provideService(InfrastructureContext, infrastructure)
        )
      )
    ),
   ...
   */
  useService: <T>(
    selector: (infrastructure: AppInfrastructure) => T
