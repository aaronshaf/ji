...
export class EffectConfigManager implements ConfigurationService {
        ...
        this.validation.required.map(key =>
          pipe(
            ...
            Effect.mapError(error => new ValidationError(
              `Required configuration missing: ${String(key)}`,
              String(key),
              undefined,
              error
            ))
  ...
  private saveConfig(): Effect.Effect<void, ConfigError> {
    return Effect.tryPromise({
      try: async () => {
        ...
        `Failed to save configuration to ${this.configPath}: ${error}`,
        error
      )
    });
  }
          ...
          pipe(
            ...
            Effect.flatMap(value => 
              pipe(
                validator(value),
                Effect.mapError(error => new ValidationError(
                  `Validation failed for ${path}: ${error.message}`,
                  path,
                  value
                ))
              )
  ...
  private startConfigWatcher(): void {
    pipe(
      ...
      Effect.repeat(this.watchSchedule),
      Effect.fork,
      Effect.runPromise
    ).catch(error => {
...
export const configValidation: ConfigValidation = {
  ...
  validators: {
    'app.environment': (value) => Effect.sync(() => {
    ...
    'jira.baseUrl': (value) => Effect.sync(() => {
    ...
    'confluence.baseUrl': (value) => Effect.sync(() => {
      if (value && typeof value === 'string' && value.length > 0) {
        try {
          new URL(value);
        } catch {
    ...
    'database.path': (value) => Effect.sync(() => {
      if (!value || typeof value !== 'string' || value.length === 0) {
        throw new ValidationError(
    ...
    'jobs.workers': (value) => Effect.sync(() => {
      if (typeof value !== 'number' || value < 1 || value > 10) {
        throw new ValidationError(
          'Job workers must be between 1 and 10',
          'jobs.workers',
          value
        );
      }
...
export const ConfigurationLayer = (configPath?: string) => Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    yield* configManager.validate().pipe(
