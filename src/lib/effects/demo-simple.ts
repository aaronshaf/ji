...
import { containsUncertainty, hashQuestion } from './memory-effect';
import { confluenceToMarkdown, normalizeText } from './text-transform-effect';
...
async function main() {
  ...
  const certain = await Effect.runPromise(containsUncertainty(certainFact));
  console.log(`  "${certainFact}" -> ${certain ? 'uncertain' : 'certain'}`);

  const uncertain = await Effect.runPromise(containsUncertainty(uncertainFact));
  ...
  const processAndHash = (content: string) =>
    pipe(
