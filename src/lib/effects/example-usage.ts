...
async function main() {
  ...
  const batchResult = await pipe(
    ...
    Effect.tap((issues) => Effect.sync(() => console.log(`Found ${issues.length} issues in cache`))),
  ...
  const updateIssueCache = (projectKey: string) =>
    pipe(
      // Get update range
      ...
      Effect.flatMap((result) =>
        pipe(
          result.issues,
          Effect.forEach((issue) => cacheEffect.saveIssue(issue), { concurrency: 5 }),
        ),
      ),
      Effect.tap((saved) => Effect.sync(() => console.log(`Saved ${saved.length} issues to cache`))),
    );

  // Run the composed operation
  await pipe(
    updateIssueCache('EVAL'),
    Effect.catchAll((error) => Effect.sync(() => console.log(`Update failed: ${error.message}`))),
    Effect.runPromise,
  );
  ...
  const checkAndDisplayIssue = (key: string) =>
    pipe(
