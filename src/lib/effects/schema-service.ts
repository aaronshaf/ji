...
export interface SchemaService {
  ...
  // Index management
  readonly createIndex: (indexName: string, tableName: string, columns: string[]) => Effect.Effect<void, QueryError | ValidationError>;
...
export class SchemaServiceTag extends Context.Tag('SchemaService')<
  SchemaServiceTag,
  SchemaService
>() {}
...
class SchemaServiceImpl implements SchemaService {
    ...
    return pipe(
      ...
      Effect.flatMap((tables) => {
        const requiredTables = [
          'config', 'projects', 'issues', 'searchable_content', 
          'content_fts', 'boards', 'user_workspaces', 'user_sprints',
          'ask_memory', 'sprint_issues_cache', 'schema_migrations'
        ];
    ...
    return pipe(
      ...
      Effect.flatMap(() => {
        ...
        if (!migration || !migration.down) {
          return Effect.fail(
            new DatabaseError(`No rollback available for migration ${version}`)
          );
        }
  ...
  // ============= Index Management =============
  createIndex(indexName: string, tableName: string, columns: string[]): Effect.Effect<void, QueryError | ValidationError> {
    return pipe(
  ...
  restoreBackup(backupPath: string): Effect.Effect<void, QueryError | ValidationError> {
    return pipe(
      this.validateBackupPath(backupPath),
      Effect.flatMap(() =>
        Effect.fail(new ValidationError('Backup restore not supported in current implementation', 'backupPath', backupPath))
      )
    );
  }
  ...
  private getForeignKeyCount(): Effect.Effect<number, QueryError> {
    return pipe(
      this.getAllTables(),
      Effect.flatMap((tables) =>
        Effect.forEach(tables, (table) => this.getTableForeignKeys(table))
      ),
      Effect.map((foreignKeyLists) => 
        foreignKeyLists.reduce((sum, fks) => sum + fks.length, 0)
      )
    );
  }
  ...
  private validateSQL(sql: string): Effect.Effect<void, ValidationError> {
    return Effect.sync(() => {
      ...
      const dangerous = ['DROP', 'DELETE', 'TRUNCATE', 'ALTER'].some(keyword =>
        sql.toUpperCase().includes(keyword) && !sql.toUpperCase().startsWith('CREATE')
      );
      if (dangerous) {
