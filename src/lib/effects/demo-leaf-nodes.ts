...
import { confluenceToMarkdown, extractCodeBlocks, extractTeamOwnership, normalizeText } from './text-transform-effect';
...
async function main() {
  ...
  for (const fact of facts) {
    await pipe(
      ...
      Effect.match({
        onFailure: (error) => console.log(`  Error: ${error.message}`),
        onSuccess: (hasUncertainty) => console.log(`  "${fact}" -> ${hasUncertainty ? 'uncertain' : 'certain'}`),
      }),
      Effect.runPromise,
    );
  }
...
This service handles authentication...
  ...
  const texts = ['Hello   World!!!', 'Testing @#$ Special-Characters', ''];
